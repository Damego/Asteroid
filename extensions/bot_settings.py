...
class Settings(commands.Cog, description='Настройка бота'):
    ...
    async def set_conf(self, ctx):
        await ctx.send('use `set prefix` or `set color`')
    ...
    async def change_guild_embed_color(self, ctx, color):
        newcolor = '0x'+color
        server[str(ctx.guild.id)]['embed_color'] = newcolor

        embed = discord.Embed(title=f'Изменился цвет сообщений бота !`', color=newcolor)
        await ctx.send(embed=embed)
