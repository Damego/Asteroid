from discord import ChannelType, Embed, Forbidden, Member, Role, VoiceChannel
...
class Moderation(Cog):
    ...
    async def mod_ban(self, ctx: SlashContext, member: Member, reason: str = None):
    ...
    async def mod_kick(self, ctx: SlashContext, member: Member, reason: str = None):
        lang = await self.bot.get_guild_bot_lang(ctx.guild_id)
    ...
    async def mod_remove_role(self, ctx: SlashContext, member: Member, role: Role):
    ...
    @bot_owner_or_permissions(manage_roles=True)
    @bot_has_guild_permissions(manage_roles=True)
    async def mod_add_role(self, ctx: SlashContext, member: Member, role: Role):
    ...
    async def mod_nick(self, ctx: SlashContext, member: Member, new_nick: str):
        lang = await self.bot.get_guild_bot_lang(ctx.guild_id)
    ...
    async def mod_clear(self, ctx: SlashContext, amount: int, member: Member = None):
        def check(message):
    ...
    @slash_subcommand(
        ...
        options=[
            create_option(
                name="member",
                description="The member for move",
                option_type=SlashCommandOptionType.USER,
            ),
            create_option(
                name="channel",
                description="The voice channel for moving member",
    ...
    async def mod_move__to(self, ctx: SlashContext, member: Member, channel: VoiceChannel):
        content = get_content(
