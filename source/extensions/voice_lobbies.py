...
from interactions import Channel, ChannelType, CommandContext, Extension, Overwrite, Permissions
...
from core import Asteroid, GuildData, GuildVoiceLobbies  # isort: skip
...
class VoiceLobbies(Extension):
    ...
    async def on_voice_state_update(self, before: VoiceState, after: VoiceState):
        ...
        if after.channel_id == voice_lobbies.voice_channel_id:
            ...
            await channel.send("There are some tips for you lmao.")  # TODO: Add text + localization
    ...
    async def __remove_user_channel(
        ...
        if not before or not before.channel_id:
            return
        lobby = guild_data.voice_lobbies.get_lobby(channel_id=int(before.channel_id))
        if not lobby:
            return
