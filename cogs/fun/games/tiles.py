...
class Tiles:
    ...
    async def start(self):
        ...
        self.message = await self.ctx.send(f"Tiles. Moves: {self.moves}", components=self._render_components())
        ...
        while True:
            ...
            if index+1 <= len(self.board) and (index+1) % 4 != 0 and self.board[index+1] == 0:
                self.board[index+1], self.board[index] = self.board[index], self.board[index+1]
