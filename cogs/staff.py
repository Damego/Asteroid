...
from my_utils import AsteroidBot, Cog, consts, paginator, transform_permission
...
class Staff(Cog):
    ...
    async def get_guild_roles(self, ctx: SlashContext, guild_id: str):
        ...
        for count, role in enumerate(guild_roles, start=1):
            if count == 1:
                embed = Embed(title=f"Roles of {guild.name} server", description="")
            if count % 25 == 0:
                embeds.append(embed)
                embed = Embed(title=f"Roles of {guild.name} server", description="")
    ...
    async def get_guild_bot_info(self, ctx: SlashContext, guild_id: str):
        ...
        embed = Embed(title=f"Bot information on {guild.name} server")
        embed.add_field(
            name="Roles",
        ...
        server_info = (
            f"**Bot's amount:** `{len([member for member in guild.members if member.bot])}`\n"
